...
APP_VERSION = "R2.1 CLOUD AUTO-DATA"
...
GEMINI_MODEL = str(get_secret("GEMINI_MODEL", "gemini-3.6-flash"))
...
from data_sources import DataResult, apply_snapshot_fallback, best_price_series, fetch_all, series_metrics  # noqa: E402
...
def load_bundle(city: str) -> Dict[str, DataResult]:
    live = fetch_all(api_key=GEMINI_API_KEY, city=city)
    snapshot_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "data", "auto_snapshot.json")
    return apply_snapshot_fallback(live, snapshot_path)
...
if page == "📊 Tổng quan tự động":
    ...
    st.info("Không nhập Excel. App tự lấy dữ liệu từ API/library, website chính thức và Google Search grounding khi có GEMINI_API_KEY. Cache 15 phút; phiên đang mở tự kiểm tra lại mỗi 15 phút.")

    @st.fragment(run_every="15m")
    def live_overview() -> None:
        live = load_bundle(city)
        keys = ["gold", "fx", "cpi", "bank_rates", "vnindex", "bds"]
        cols = st.columns(6)
        for col, key in zip(cols, keys):
            r = live[key]
            col.metric(r.name, f"{status_icon(r.status)} {r.status}")
        st.caption(f"Lần dựng màn hình: {datetime.now().astimezone().strftime('%H:%M:%S %d/%m/%Y')}")
...
st.divider()
st.caption("R2.1 CLOUD AUTO-DATA • GitHub là source code, Streamlit Community Cloud là runtime. Dữ liệu thị trường lấy tự động; không dùng SQLite/Excel làm nguồn chính.")
